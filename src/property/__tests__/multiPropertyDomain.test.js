...
import {
  ...
  getPropertyInsuredAddressError,
...
describe("multiPropertyDomain", () => {
  it("returns field-level PSAKI contact validation messages", () => {
    assert.deepEqual(
      getPropertyContactFieldErrors({
        identity: "",
        phone: "123",
      ...
      {
        identity: "Isi nama calon pemegang polis atau pilih CIF.",
        phone: "Nomor handphone harus berisi 10-15 digit.",
        email: "Alamat email harus menggunakan format nama@domain.",
      },
    );
  ...
  it("returns field-level PSAKI identity number validation messages", () => {
    assert.equal(getPropertyIdNumberError("Nasabah Perorangan", "123", true), "NIK harus berisi 16 digit.");
    assert.equal(getPropertyIdNumberError("Badan Usaha", "123", true), "NPWP minimal 15 digit.");
    assert.equal(getPropertyIdNumberError("Nasabah Perorangan", "3174010101010001", true), "");
  });

  it("returns field-level PSAKI insured address validation messages", () => {
    assert.equal(
      getPropertyInsuredAddressError({ insuredAddress: "", sameAsPropertyAddress: false, propertyLocation: "Jl. Sudirman", showRequired: true }),
      "Isi alamat calon pemegang polis.",
    );
    assert.equal(
      getPropertyInsuredAddressError({ insuredAddress: "", sameAsPropertyAddress: true, propertyLocation: "", showRequired: true }),
      "Isi alamat objek pertanggungan terlebih dahulu.",
    );
    assert.equal(
      getPropertyInsuredAddressError({ insuredAddress: "", sameAsPropertyAddress: true, propertyLocation: "Jl. Sudirman", showRequired: true }),
      "",
    );
  });
