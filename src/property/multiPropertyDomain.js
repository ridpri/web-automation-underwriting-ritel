...
export function getPropertyContactFieldErrors({ identity = "", phone = "", email = "", showRequired = false } = {}) {
  const errors = {};
  const trimmedIdentity = String(identity || "").trim();
  const trimmedPhone = String(phone || "").trim();
  const trimmedEmail = String(email || "").trim();

  if (showRequired && !trimmedIdentity) errors.identity = "Isi nama calon pemegang polis atau pilih CIF.";
  if (!trimmedPhone && showRequired) errors.phone = "Nomor handphone wajib diisi.";
  else if (trimmedPhone && !isValidPhoneNumber(trimmedPhone)) errors.phone = "Nomor handphone harus berisi 10-15 digit.";

  if (!trimmedEmail && showRequired) errors.email = "Alamat email wajib diisi.";
  else if (trimmedEmail && !isValidEmailAddress(trimmedEmail)) errors.email = "Alamat email harus menggunakan format nama@domain.";
...
export function getPropertyIdNumberError(customerType, value, showRequired = false) {
  const digits = onlyDigits(value);
  if (!digits && !showRequired) return "";
  if (customerType === "Badan Usaha") return digits.length >= 15 ? "" : "NPWP minimal 15 digit.";
  return digits.length === 16 ? "" : "NIK harus berisi 16 digit.";
}

export function getPropertyInsuredAddressError({ insuredAddress = "", sameAsPropertyAddress = false, propertyLocation = "", showRequired = false } = {}) {
  if (!showRequired) return "";
  if (sameAsPropertyAddress) {
    return String(propertyLocation || "").trim() ? "" : "Isi alamat objek pertanggungan terlebih dahulu.";
  }
  return String(insuredAddress || "").trim() ? "" : "Isi alamat calon pemegang polis.";
}
