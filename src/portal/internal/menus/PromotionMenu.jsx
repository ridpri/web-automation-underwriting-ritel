...
export default function PromotionMenu() {
  ...
  const openManagePromo = (promo) => {
    setEditingPromo(promo);
    setSelectedProducts(promo.products.split(",").map((item) => item.trim()).filter(Boolean));
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="rounded-xl border border-[#D9E1EA] bg-[#F6F8FA] p-3 shadow-sm">
        <div className="overflow-hidden rounded-xl border border-[#D9E1EA] bg-white">
          <table className="w-full min-w-[980px] text-left text-[12px]">
            ...
            <tbody className="divide-y divide-[#E7EDF4] text-[#041E42]">{rows.map((promo) => <tr key={`${promo.code}-${promo.period}`} className="hover:bg-[#F8FAFC]"><td className="px-4 py-4 font-medium">{promo.code}</td><td className="px-4 py-4">{promo.products}</td><td className="px-4 py-4">{promo.discount}</td><td className="px-4 py-4">{promo.quota}</td><td className="px-4 py-4">{promo.period}</td><td className="px-4 py-4"><StaffBadge>{promo.status}</StaffBadge></td><td className="px-4 py-4"><button type="button" onClick={() => openManagePromo(promo)} aria-label={`Kelola promo ${promo.code}`} className="inline-flex h-8 items-center gap-2 rounded-lg border border-[#D9E1EA] bg-white px-3 text-[12px] font-medium text-[#004B78] hover:bg-[#EEF5FA]">Kelola<ChevronDown className="h-3.5 w-3.5" /></button></td></tr>)}</tbody>
          </table>
        </div>
      </section>
      ...
      {modalOpen ? <div className="fixed inset-0 z-50 grid place-items-center bg-[#041E42]/35 px-4 py-6"><div className="w-full max-w-[570px] overflow-hidden rounded-xl border border-[#D9E1EA] bg-white shadow-2xl"><div className="flex items-center justify-between border-b border-[#E7EDF4] px-4 py-3"><div className="inline-flex items-center gap-2 text-[15px] font-black text-[#041E42]"><CreditCard className="h-4 w-4 text-[#004B78]" />{editingPromo ? `Kelola ${editingPromo.code}` : "Tambah Promo Code"}</div><button type="button" onClick={closePromoModal} className="h-8 rounded-lg border border-[#D9E1EA] bg-white px-3 text-[12px] font-bold text-[#304B68] hover:bg-[#F8FAFC]">Tutup</button></div><div className="grid gap-4 px-4 py-4"><label className="block"><span className="text-[11px] font-black uppercase tracking-[0.18em] text-[#9AAAC0]">Produk</span><div className="mt-2"><PromoProductPicker selected={selectedProducts} onToggle={toggleProduct} /></div></label><label className="block"><span className="text-[11px] font-black uppercase tracking-[0.18em] text-[#9AAAC0]">Kode Promo</span><div className="mt-2 flex rounded-lg border border-[#D9E1EA] bg-white"><input value={form.code} onChange={(event) => updateForm("code")(event.target.value)} className="h-10 min-w-0 flex-1 rounded-lg border-0 bg-transparent px-3 text-[13px] font-black text-[#041E42] outline-none" /><button type="button" className="m-1 rounded-md border border-[#D9E1EA] px-3 text-[11px] font-black text-[#004B78] hover:bg-[#EEF5FA]">Generate Ulang</button></div></label><div className="grid gap-3 md:grid-cols-[124px_minmax(0,1fr)]"><StaffField label="Kuota" value={form.quota} onChange={updateForm("quota")} /><StaffField label="Diskon (%)" suffix="%" value={form.percent} onChange={updateForm("percent")} /></div><StaffField label="Periode Promo" value={form.period} onChange={updateForm("period")} /><label className="flex items-center justify-between gap-4 rounded-lg border border-[#D9E1EA] bg-[#F8FAFC] px-3 py-3"><span className="text-[12px] font-black text-[#041E42]">Status Aktif</span><button type="button" onClick={() => setActive((current) => !current)} className={cls("relative h-8 w-14 rounded-full transition", active ? "bg-[#004B78]" : "bg-[#CBD5E1]")}><span className={cls("absolute top-1 h-6 w-6 rounded-full bg-white shadow transition", active ? "left-7" : "left-1")} /></button></label></div><div className="flex justify-end gap-2 border-t border-[#E7EDF4] bg-white px-4 py-3"><button type="button" onClick={closePromoModal} className="h-9 rounded-lg border border-[#D9E1EA] bg-white px-4 text-[12px] font-bold text-[#004B78] hover:bg-[#EEF5FA]">Batal</button><button type="button" onClick={closePromoModal} className="h-9 rounded-lg bg-[#9AAAC0] px-4 text-[12px] font-bold text-white">{editingPromo ? "Simpan Perubahan" : "Simpan Promo"}</button></div></div></div> : null}
    </div>
  );
}
