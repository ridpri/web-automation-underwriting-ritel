...
export function getMinimumVehicleCoverageEndDate(coverageStartDate = "") {
  return addMonths(coverageStartDate, 6);
}

export function getVehicleCoveragePeriodError(coverageStartDate = "", coverageEndDate = "") {
  if (!coverageStartDate) return "Tanggal mulai perlindungan belum diisi.";
  if (!coverageEndDate) return "Tanggal akhir perlindungan belum diisi.";
  const minimumEndDate = getMinimumVehicleCoverageEndDate(coverageStartDate);
  if (!minimumEndDate || coverageEndDate < minimumEndDate) return "Jangka waktu pertanggungan minimal 6 bulan.";
  return "";
}
...
export function getMultiVehicleStepTwoPendingItems({ flowType = "motor", customerType = "Pribadi", idNumber = "", insuredName = "", address = "", email = "", phone = "", coverageStartDate = "", coverageEndDate = "", vehicles = [] } = {}) {
  const items = [];
  ...
  const periodError = getVehicleCoveragePeriodError(coverageStartDate, coverageEndDate);
  if (periodError) items.push(periodError);
  vehicles.forEach((vehicle, index) => {
    const label = vehicleLabel(vehicle, index);
