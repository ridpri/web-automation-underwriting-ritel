...
import { PLATES, getExtensionFee, getRegion } from "../motorDomain.js";
...
import {
  ...
  getMinimumVehicleCoverageEndDate,
...
const VEHICLE_USAGE_HELP_TEXT = "Pilih Pribadi bila kendaraan dipakai untuk keperluan pribadi. Pilih Komersial bila kendaraan disewakan atau digunakan untuk menerima balas jasa.";
...
function TextInput({ value, onChange, placeholder, icon, type = "text", inputMode, listId, min }) {
  return (
...
function VehicleSumInsuredHint() {
  const [open, setOpen] = useState(false);
  const termClass = "rounded-[4px] px-0.5 font-semibold text-[#0A4D82] underline decoration-[#0A4D82]/30 underline-offset-2 hover:bg-[#EAF4FD] focus:bg-[#EAF4FD] focus:outline-none focus:ring-2 focus:ring-[#0A4D82]/20";
  const trigger = (
    <button
      type="button"
      className={termClass}
      ...
      aria-expanded={open}
    >
      harga jual kendaraan Anda saat ini
    </button>
  );

  return (
    <div className="relative mt-1.5" onMouseLeave={() => setOpen(false)}>
      <div className="text-[11px] leading-4 text-slate-500">
        Isi sesuai perkiraan {trigger}.
      </div>
      {open ? (
        <div className="absolute bottom-[calc(100%+6px)] right-0 z-30 w-[min(760px,calc(100vw-48px))] rounded-[10px] border border-[#C8D6E5] bg-white px-3 py-2 text-[11px] leading-4 text-slate-600 shadow-[0_14px_34px_rgba(15,23,42,0.16)]">
          <div className="font-semibold text-slate-800">Harga jual kendaraan Anda saat ini</div>
          <div className="mt-1">Perkiraan nilai kendaraan di pasar bebas dengan merek, tipe, model, tahun, dan kondisi yang sama.</div>
          <div className="mt-1.5 font-medium text-slate-700">Jika nilainya terlalu rendah, klaim dapat dibayar secara proporsional. Jika terlalu tinggi, premi bisa lebih besar, tetapi pembayaran klaim tetap mengikuti nilai kendaraan yang sebenarnya sesuai polis.</div>
        </div>
      ) : null}
    </div>
  );
...
function VehicleUsageHint({ usage }) {
  const [open, setOpen] = useState(false);
  if (usage !== "Pribadi" && usage !== "Komersial") return null;
  const content = usage === "Pribadi"
    ? {
        title: "Penggunaan Pribadi",
        summary: "Digunakan untuk keperluan pribadi dan bukan untuk disewakan atau menerima balas jasa.",
        consequence: "Contoh yang bukan penggunaan pribadi: ojek, kurir berbayar, layanan antar berbayar, atau sewa kendaraan.",
      }
    : {
        title: "Penggunaan Komersial",
        summary: "Digunakan untuk disewakan atau menerima balas jasa.",
        consequence: "Contohnya ojek, kurir berbayar, layanan antar berbayar, atau sewa kendaraan.",
      };
  const triggerClass = "rounded-[4px] px-0.5 font-semibold text-[#0A4D82] underline decoration-[#0A4D82]/30 underline-offset-2 hover:bg-[#EAF4FD] focus:bg-[#EAF4FD] focus:outline-none focus:ring-2 focus:ring-[#0A4D82]/20";
  ...
  return (
    <div className="relative mt-1.5" onMouseLeave={() => setOpen(false)}>
      ...
      {open ? (
        <div className="absolute bottom-[calc(100%+6px)] left-0 z-30 w-[min(520px,calc(100vw-48px))] rounded-[10px] border border-[#C8D6E5] bg-white px-3 py-2 text-[11px] leading-4 text-slate-600 shadow-[0_14px_34px_rgba(15,23,42,0.16)]">
          <div className="font-semibold text-slate-800">{content.title}</div>
          <div className="mt-1 font-medium text-slate-700">{content.consequence}</div>
        </div>
      ) : null}
    </div>
  );
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="md:col-span-2">
              <div className="md:max-w-[760px]">
                ...
                <VehicleUsageHint usage={vehicle.quote.usage} />
                {tariffInfoSummary ? (
  ...
  const stepTwoPendingItems = useMemo(
    () =>
      getMultiVehicleStepTwoPendingItems({
        ...
        coverageEndDate: policyForm.coverageEndDate,
        vehicles,
      }),
    [flowType, policyForm.address, policyForm.coverageEndDate, policyForm.coverageStartDate, policyForm.customerType, policyForm.email, policyForm.idNumber, policyForm.insuredName, policyForm.phone, vehicles],
  );
          ...
          <div className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                ...
                <TextInput
                  value={policyForm.coverageStartDate || ""}
                  onChange={(value) => updatePolicy({ coverageStartDate: value })}
                  type="date"
                />
              </div>
              <div>
                <FieldLabel label="Jangka Waktu Pertanggungan (Akhir)" required />
                <TextInput
                  value={policyForm.coverageEndDate || ""}
                  onChange={(value) => updatePolicy({ coverageEndDate: value })}
                  type="date"
                  min={getMinimumVehicleCoverageEndDate(policyForm.coverageStartDate)}
                />
                {policyForm.coverageStartDate ? <div className="mt-1.5 text-[11px] leading-4 text-slate-500">Minimal 6 bulan sejak tanggal mulai. Boleh lebih dari 1 tahun.</div> : null}
              </div>
            </div>
            {vehicles.map((vehicle, index) => (
    ...
    <div className="space-y-5">
      <ActionCard>
        <div className="text-center">
          ...
          <div className="mx-auto mt-2 max-w-2xl text-sm leading-6 text-slate-500 md:text-[15px]">
            Data simulasi ini disusun sebagai bagian awal dari SPAU (Surat Permohonan Asuransi Umum) elektronik yang Anda isi dan lengkapi, serta mengacu pada Polis Standar Asuransi Kendaraan Bermotor Indonesia sebagai dasar simulasi premi dan langkah berikutnya.
          </div>
        </div>
      </ActionCard>
      <ActionCard>
